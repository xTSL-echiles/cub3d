...
int		ft_acces_checker(t_options *qu)
{
	...
	{	
		...
		&& ((qu->cr > -1) && (qu->cr <= 255)) && ((qu->cg > -1) && (qu->cg <= 255))
		&& ((qu->cb > -1) && (qu->cb <= 255)))
		{
...
void check_texture(t_options *qu, char *line)
{
	char **ans;
	int i;
	...
	if (i == 2)
	{
		if ((ft_strrchr(ans[0], 'E') != NULL) && (ft_strrchr(ans[0], 'A') != NULL))
			qu->ea = ft_strcpy(ans[1]);
		else if ((ft_strrchr(ans[0], 'W') != NULL) && (ft_strrchr(ans[0], 'E') != NULL))
			qu->we = ft_strcpy(ans[1]);
		else if ((ft_strrchr(ans[0], 'N') != NULL) && (ft_strrchr(ans[0], 'O') != NULL))
			qu->no = ft_strcpy(ans[1]);
		else if ((ft_strrchr(ans[0], 'S') != NULL) && (ft_strrchr(ans[0], 'O') != NULL))
...
void check_fc(t_options *qu, char *line)
{
	char **ans;
	int i;
